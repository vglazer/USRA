% SYNTAX
% testfd(appord, testfno, ntimes, thresh)
%
% DESCRIPTION
% testfd is used to test the convergence properties of the finite difference 
% formulas for the first derivative found in approxder. 
%
% PARAMETERS
...
% EXAMPLES
% testfd(21,31,4) 
%   test the nonuniform O(h^2) formulas on x^4 using four grids of sizes 
%   64, 128, 256 and 512. Use the default error threshhold, i.e. flag 
%   gridpoints where the error is > 10^0 = 1.
%
% testfd(32,1012,5,-3) 
%   test the Newton form of the nonuniform O(h^3) formulas on function 
%   904 using five grids of sizes 64-1024. Flag gridpoints where the error 
%   is > 10^(-3) = 0.001.
function testfd(appord, testfno, ntimes, thresh)
...
for nn = 1:ntimes
    ...
    [truevals,dervals] = truevd(nugridx);
    appvals = approxder(nugridx,truevals,appord);
    errvals = abs(dervals - appvals);
    maxerr = max(errvals);
...
disp([12 num2str(ind) '/' num2str(n)  ' = ' num2str(ind/n*100) '% points ' ... 
     'where error in derivative approximation > ' num2str(10^thresh)]);
clf;
hold on;
if (ind > 0), plot(badpts,truevd(badpts),'o');, end
plot(nugridx,truevd(nugridx));
title(['Points where error in derivative approximation > ' num2str(10^thresh)]);
hold off;
