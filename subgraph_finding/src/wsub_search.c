/* 
   use randomised restricted local search to find vertex-induced subgraphs 
   with a prescribed edge count in undirected, weighted graphs (where the 
   weights are assumed to be small integers); the weights are encoded as 
   follows: if the ith row of the adjacency list contains entry x, then 
   {i, (x % v)} is an edge with weight (x / v) + 1 (in other words, if there 
   is an edge of weight w between i and j, we add entry j + (w - 1) * v to the
   ith row of the adjacency list)
...
int main(void) {
    ...
    for (i = 0; i < v; i++) 
        for (;;) {
            ...
            degree = vert % v; temp = vert / v + 1;
            adj[i][degree] = adj[degree][i] = temp;
        }      

    /* fill in adjacency list, count degrees and edges */
    for (i = e = 0; i < v; i++) {  
        for (j = k = 0; j < v; j++) 
            if ((temp = adj[i][j])) { vert_inds[k++] = j; e += temp; }
        adj_list[i] = malloc(k * sizeof(int));
        for (j = 0; j < k; j++) adj_list[i][j] = vert_inds[j]; degs[i] = k; 
    }   
    e /= 2;
    ...
    for (i = 0; i < num_exps; i++) {
        ...
        /* begin moves */       
        opt_val = g_opt_val = old_min_val = e; opt_mov = obj_val = 0;
    	for (j = 0; j < num_mov; j++) {
            ...
            for (k = 0; k < degs[sub_vert]; k++) {
                ...
                sub_degs[vert] -= adj[sub_vert][vert]; u_ch[vert] = 1;
            }
            for (k = 0; k < degs[rest_vert]; k++) {
                vert = adj_list[rest_vert][k];
                sub_degs[vert] += adj[rest_vert][vert];
    ...
    free(sub_opt); free(rest_opt); free(u_list); free(degs); free(sub_degs);
    free(vert_inds); free(sub_ch); free(u_ch); free(boxes); free(rand_perm);
